...
$(document).ready(function(){
	...
	$('#form_abono').on('submit', function guardarAbono(event){
		...
		datos+= "&id_recaudaciones=" + $("#sesion_id_recaudaciones").val();
		// datos.push({
		// name: "id_recaudaciones",
		// value: $("#sesion_id_recaudaciones").val()
		// });
		boton.prop('disabled',true);
		icono.toggleClass('fa-save fa-spinner fa-pulse ');
		
		$.ajax({  
			url: 'control/guardar_abono.php',
			method: 'POST', 
			dataType: 'JSON',
			data: datos
			}).done(function(respuesta){ 
...
function imprimirTicket(event){
	...
	$.ajax({
		...
		if($("#silent_print").val() == "SI" ){
			...
			if($("#sesion_id_usuarios").val() == "56"  || $("#sesion_id_usuarios").val() == "59" || $("#sesion_id_usuarios").val() == "61" // alexis){
				$.ajax({
